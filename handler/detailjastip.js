            ...
            .then((docSnap) => {
                if (docSnap.exists()) {
                    ...
                    orderButton.addEventListener("click", () => {
                        saveOrderToFirestore(product, "Diterima");
                    });
...
async function saveOrderToFirestore(product, orderStatus) {
    try {
        ...
        const orderData = {
            ...
            harga: product.harga,     
            nama_jastip: product.nama_jastip,  
            statuspengiriman: orderStatus,       
            orderTimestamp: serverTimestamp(),        
        };

        await addDoc(ordersRef, orderData);        
        console.log("Pesanan berhasil disimpan:", orderData);

        // Setelah pesanan disimpan, arahkan ke halaman detailpesanan.html
        window.location.href = `detailpesanan.html?id=${product.id_produk}`;  // Arahkan pengguna ke halaman detail pesanan
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        console.log(`Pengguna ${user.uid} login.`);
    } else {
